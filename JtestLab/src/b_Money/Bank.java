...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			accountlist.put(accountid, new Account(accountid,currency));  //Bug: Was not actually creating a new account because our test cases gave null object
		}
	}
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {  //Bug: deposit test case showed that this if statement was throwing an error if it existed instead if it didn't exist
	...
	public void withdraw(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) {
			...
			Account account = accountlist.get(accountid);
			account.withdraw(money);  //Bug: Withdraw test case showed that when withdrawing from an account with 0, it returned a positive value instead of negative. Knew it was adding instead of subtracting.
		}
	}
